...
local function findAvailableSpawnPoint(baseSpawn)
    ...
    for i = 1, #candidates do
        ...
        if not Config.distanceCheck or not lib.getClosestVehicle(candidate.xyz, Config.distanceCheck, false) then
            return candidate
        end
    end
end
...
lib.callback.register('qbx_garages:server:spawnVehicle', function (source, vehicleId, garageName, accessPointIndex)
    ...
    local spawnCoords = findAvailableSpawnPoint(accessPoint.spawn or accessPoint.coords)
