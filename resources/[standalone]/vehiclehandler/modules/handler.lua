...
function Handler:constructor()
    ...
    self.private.bazufixfuel = GetResourceState('bazufix-fuel') == 'started' and true or false
end
...
---@return boolean bazufixfuel
function Handler:isFuelBazufix() return self.private.bazufixfuel end

---@param vehicle number
---@param level number
function Handler:setFuelLevel(vehicle, level)
    if self:isFuelOx() then
        Entity(vehicle).state.fuel = level
    end

    if self:isFuelBazufix() then
        DecorSetFloat(vehicle, 'bzfuel_level', level)
    end
...
function Handler:adminfix()
    ...
    if success then
        lib.callback('vehiclehandler:sync', false, function()
            ...
            self:setFuelLevel(cache.vehicle, 100.0)
...
function Handler:adminfuel(newlevel)
    ...
    if success then
        ...
        lib.callback('vehiclehandler:sync', false, function()
            self:setFuelLevel(cache.vehicle, newlevel)
